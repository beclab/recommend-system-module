...
func (s *Service) HandleRssQuery(c *gin.Context) {
	...
	var rssQuery RssQueryRequest
	log.Debug().Msgf("request body: %s", string(body))
	err = json.Unmarshal(body, &rssQuery)

	log.Info().Msgf("zinc query index %s term %s max %v", index, rssQuery.Query, rssQuery.Limit)
	//res, err := s.ZincRawQuery(index, term, int32(maxResults))
	res, err := s.EsRawQuery(index, rssQuery.Query, rssQuery.Limit)
	if err != nil {
