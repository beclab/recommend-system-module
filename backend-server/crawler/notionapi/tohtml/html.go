...
import (
	...
	"bytetrade.io/web3os/backend-server/crawler/notionapi"
)
...
func (c *Converter) RenderImage(block *notionapi.Block) {
	c.Printf(`<figure id="%s" class="image">`, block.ID)
	{
		...
		uri := "https://pmthinking.notion.site/image/" + url.QueryEscape(block.Source) + "?table=block&id=" + block.ID + "&spaceId=" + block.SpaceID
